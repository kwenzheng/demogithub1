...
int process_user_modify_request(int acceptfd,MSG *msg)
{
	...
	switch(msg->flags)
	{
		case 1:
			...
			printf("modify usrinfo table success.\n");
			send(acceptfd,msg,sizeof(MSG),0);
			}
			break;
		case 2:
...
int process_user_query_request(int acceptfd,MSG *msg)
{
	...
	if(sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK){
		...
		printf("query usrinfo table success.\n");
		send(acceptfd,msg,sizeof(MSG),0);
	}
}
...
int process_admin_modify_request(int acceptfd,MSG *msg)
{
	...
	switch(msg->flags)
	{
		...
		case 5:
			...
			break;
		case 6:
			sprintf(sql,"update usrinfo set salary=%.1lf  where staffno=%d;",msg->info.salary,msg->info.no);
			if(sqlite3_exec(db,sql,NULL,NULL,&errmsg)!= SQLITE_OK){
				printf("%s.\n",errmsg);
			}else{
			printf("modify usrinfo table success.\n");
